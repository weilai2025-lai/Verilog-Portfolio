...
ARCHITECTURE structure OF count111_mealy IS
...
SIGNAL \current.s2~feeder_combout\ : std_logic;
...
SIGNAL \current.s1~0_combout\ : std_logic;
...
\current.s2~feeder\ : cycloneiv_lcell_comb
-- Equation(s):
-- \current.s2~feeder_combout\ = \current.s1~q\
...
PORT MAP (
	datac => \current.s1~q\,
	combout => \current.s2~feeder_combout\);
...
-- Equation(s):
-- \Selector0~0_combout\ = (\one_in~input_o\) # ((\current.s1~q\) # (\current.s2~q\))
...
\current.s1~0\ : cycloneiv_lcell_comb
-- Equation(s):
-- \current.s1~0_combout\ = !\current.s0~q\
...
-- Equation(s):
-- \Selector5~0_combout\ = (\one_in~input_o\ & !\current.s1~q\)
...
-- Equation(s):
-- \Selector4~0_combout\ = (\one_in~input_o\ & \current.s0~q\)
